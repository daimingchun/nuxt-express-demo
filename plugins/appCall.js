...
// import cookie from '~/plugins/cookies.js'
...
let appCall = {
    active(opt) {
        ...
        opt.url = opt.url ? opt.url : 'ciw://home';
        if (!opt.appsign) {
        ...
        if (opt.url && !this.isAppUrl(opt.url)) {//wap链接拼接app标识
            if (opt.appsign == 'ybs') {
        ...
        var cookie = VueCookie.get(opt.cookieName);
        //是到达有次数限制
        var cooklimit = opt.limitopen && cookie
        if (!iswap || cooklimit) {
            return false;
        }
        VueCookie.set(opt.cookieName, 'jhh', {expires: 1, domain: 'localhost'});
        this.openApp(opt);
    },
    /**
     * 打开app
